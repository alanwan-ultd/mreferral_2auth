...
class Admin
{
	...
	public function getListCMS($name)
	{
		...
		foreach ($rst as $key => $row) {
			switch ($row['status']) {
				case 'A':
					$rst[$key]['status'] = renderListStatusActive('Active');
					break;
				case 'P':
					$rst[$key]['status'] = renderListStatusPending('Pending');
					break;
			}
	...
	public function login($login, $password, $authCode)
	{
		...
		$rst = $db->select(
			$setting->DB_PREFIX . 'adminuser u LEFT JOIN ' . $setting->DB_PREFIX . 'admingroup g ON u.groupId=g.id',
