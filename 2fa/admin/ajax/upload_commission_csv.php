...
if (isset($_FILES['csvFile']) && $_FILES['csvFile']['error'] == UPLOAD_ERR_OK) {
	...
	while (($row = fgetcsv($csvFile)) !== false) {
		...
		$commissionData['drawdown_date'] = $row[6];
		$commissionData['property_addr'] = $row[7];
		$commissionData['payment_date'] = $row[8];
		$commissionData['staff_no'] = $row[9];
		$commissionData['staff_name'] = $row[10];
		$commissionData['staff_bank_acc'] = $row[11];
		$commissionData['comm_to_pc'] = $row[12];
		$commissionData['department_head_id'] = $row[13];
		$commissionData['department_head_name'] = $row[14];
		$commissionData['department_head_bank_account'] = $row[15];
		$commissionData['commission_branch_mgr'] = $row[16];
		$commissionData['branch'] = $row[17];
