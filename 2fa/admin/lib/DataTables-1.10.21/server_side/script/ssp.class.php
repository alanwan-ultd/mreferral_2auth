...
class SSP {
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					if(!empty($column['db'])){
						if(isset($column['regex']) && $column['regex'] == false){
	...
	static function simple ( $request, $conn, $table, $primaryKey, $columns, $whereCustom = '' )
	{
        ...
        }//var_dump("SELECT `".implode("`, `", self::pluck($columns, 'db'))."`FROM $table$where$order$limit");var_dump($bindings);//debug use
