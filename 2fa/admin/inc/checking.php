...
if(isset($name)){
	$sectionModel = new Section($name, (isset($group))?$group:'', (isset($groupSub))?$groupSub:'');
	$sectionPermission = $sectionModel->getPermissionBySection($name);
	$mySection = $sectionModel->run();
	if(isset($_SESSION['sales_groupId']) && $_SESSION['sales_groupId'] > 0){  //logined
		if(!$sectionModel->checkCanView($sectionPermission)){  //all permissions are 0, read write, approve
			if(isset($sectionPermissionByPass) && $sectionPermissionByPass === true
				&& isset($_SESSION['sales_id']) && (intval($_SESSION['sales_id']) === intval($util->returnDataNum('id', 'request')))
			){
	...
	if($checkingSearchResult == 0 || $checkingSearchResult == 1){
		if(isset($_SESSION['sales_id']) && intval($_SESSION['sales_id']) > 0){
			//do nothing
